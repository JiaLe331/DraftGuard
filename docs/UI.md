...
- Every SI/BL value carries a text-and-icon extraction label: **Rule extracted**, **AI text extracted**, **AI visual candidate**, or **Human corrected**. Visual candidates additionally show confirmation state. The evidence panel embeds the actual candidate PDF page, identifies machine raw/normalized values and page, and labels model excerpts as unconfirmed visual evidence.
- Current local tasks allow **Correct extraction** for rule, Gemini text, and Gemini visual results. Text corrections select one visible source unit and the entered value must occur in that unit; visual corrections select an actual page. Only Gemini visual candidates expose **Confirm candidate**. Busy state blocks duplicate submission, form errors stay adjacent, success uses `aria-live`, and the latest unverified demo actor is visible.
...
- Before completion, an actionable current local comparison can open **Next action · Amendment email**. The native dialog labels AI-polished versus standard wording and the exact revision/run; recipient, subject, opening, and closing are editable, while the deterministic issue list remains locked and source-backed. Copy saves dirty wording first and places a complete plain-text email on the clipboard. Gemini errors retain any prior draft and offer Retry plus an explicit standard-template choice. Clipboard failure focuses and selects the full preview for manual copy. No mail client is opened and no email is sent.
...
Run the frontend checks in README. Component tests exercise server totals/pagination, retry without fixture fallback, empty import, actual evidence links, scan-candidate states, embedded page loading, confirmation/correction and supply forms, amendment generation/edit/copy/fallback, completion eligibility/dialog/success, reviewed report ledger, focus movement, historical read-only state, failed rerun recovery, and stale-response handling. Backend tests own the authoritative document, comparison, amendment-fact, and completion rules.

This UI is backed by a local development mailbox. The Inbox separates **Recent local tasks** from the **Sample mailbox** and uses one primary **Create local task** dialog with visible labels, inline validation, duplicate-submit protection, unsaved-close confirmation, and heading focus after navigation. Samples are explicitly read only and expose only **Create working copy** for mutation. Gemini classification/text/scan paths require explicit server configuration for real calls; review, completion, and reports are local and persisted in SQLite. Multi-user sessions, cloud persistence, and the PRD's public deployment gate remain future work. Development runtime data is not part of the Git changes.
...
Sample workspaces are read only and offer creation of a local task copy. Inbox lists recent local tasks separately from baseline examples and can also create an empty custom task. Copies start without analysis results, use the shared pipeline, and preserve the baseline. Task source controls allow adding/replacing SI or BL and explicit pairing; successful changes save a new revision before requesting analysis. Failed analysis submission leaves a retryable saved revision.
